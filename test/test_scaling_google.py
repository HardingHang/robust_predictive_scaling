...
class AlibabaResourceScalingTestTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        df = data[(data['time_idx'] > 1700) & (data['time_idx'] < 1700 + data_length)]
        ...
        # self.test = test_template.generate_instances(
        #     prediction_length=self.prediction_length,
        #     windows=train_test_offset//self.prediction_length,
        # )
    ...
    def test_adaptive_robust_scaling(self):
            ...
            for j in range(i, len(param_values)):
                ...
                for test_id in range(num_test_windows):
                    ...
                    plan = manager.adaptive_robust_solution(
                        single_prediction, quantiles, thresholds=threshold)
                    ...
                    robust_under_utilization_duration_total += robust_under_utilization_duration
                # 根据参数组合计算指标值
                metric1_value = robust_upd_total / (num_test_windows*self.prediction_length) *100
                metric2_value = robust_under_utilization_duration_total / (num_test_windows*self.prediction_length) *100
                # 填充矩阵
        ...
        df_metrics1 = pd.DataFrame(matrix_metrics1, index=param_values, columns=param_values)
        df_metrics2 = pd.DataFrame(matrix_metrics2, index=param_values, columns=param_values)


        sns.set(font_scale=1.3)
        fig, axs = plt.subplots(1, 2, figsize=(12.8, 6))
        
        sns.heatmap(df_metrics1, annot=True, fmt=".2f", cmap="YlGnBu", ax=axs[0])
        axs[0].set_title('Under-Provisioning Rate (%)')
        axs[0].set_xlabel('Optional Quantile Levels')
        axs[0].set_ylabel('Optional Quantile Levels')

        sns.heatmap(df_metrics2, annot=True, fmt=".2f", cmap="YlGnBu", ax=axs[1])
        axs[1].set_title('Over-Provisioning Rate (%)')
        axs[1].set_xlabel('Optional Quantile Levels')
        axs[1].set_ylabel('Optional Quantile Levels')

        plt.tight_layout()
        plt.savefig("adaptive google tft.pdf")
        plt.show()
    ...
    def test_sensitivity_analysis(self):
        ...
        for quantiles in [[0.95, 0.9], [0.95, 0.8], [0.9, 0.8], [0.9, 0.7]]:
            ...
            plt.plot(np.arange(5, 15, 0.5), upd, color= colors[cnt], marker = markers[cnt],label=quantiles)
            # plt.plot(np.arange(5, 15, 0.5), opd, color= colors[cnt], marker = markers[cnt],label=quantiles)
        ...
        # plt.ylabel('Over-Provisioning\n Rate (%)',fontdict={'fontsize': 16})
        plt.ylabel('Under-Provisioning\n Rate (%)',fontdict={'fontsize': 16})
        fig.tight_layout()
        
        plt.savefig("sensitivity google deepar under.pdf")
        # plt.savefig("sensitivity google deepar over.pdf")
        plt.show()
